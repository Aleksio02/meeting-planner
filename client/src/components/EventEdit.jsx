  ...
  return (
    <>
      ...
      <div className="edit-modal">
        ...
        <form className="edit-form" onSubmit={(e) => { e.preventDefault(); onSave({ id: eventData?.id, ...formData }); }}>
          <h2 className="edit-header">{eventData ? 'Редактирование' : 'Новое событие'}</h2>

          <div className="edit-group">
            <label>Название события</label>
            <input name="title" value={formData.title} onChange={handleChange} className="edit-input" placeholder="Введите название" />
          </div>
          ...
          <div className="edit-group">
            ...
            <textarea name="description" value={formData.description} onChange={handleChange} className="edit-input edit-textarea" placeholder="Добавьте описание события" />
          </div>

          <div className="edit-footer">
            <button type="button" className="edit-btn-secondary" onClick={onClose}>Отмена</button>
            <button type="submit" className="edit-btn-primary">{eventData ? 'Сохранить' : 'Создать'}</button>
          </div>
        </form>
      </div>
    </>

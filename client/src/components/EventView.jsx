...
const EventView = ({ isOpen, onClose, eventId }) => {
  ...
  const loadEventData = async (id) => {
    ...
    try {
      ...
      const { data: invites } = await invitesAPI.getList({
        meetingId: id,
        page: 0,
        pageSize: 100,
      });
      ...
      const mappedParticipants = invites.map(inv => ({
        ...
        status: inv.status === 'Ожидает' ? 'pending' : 
                inv.status === 'Принято' || inv.status === 'ACCEPTED' ? 'accepted' : 'declined',
  ...
  return (
    <>
      ...
      <div className={`event-modal ${isClosing ? 'closing' : ''}`}>
        ...
        {loading ? (
          <div className="event-content" style={{ textAlign: 'center', color: '#fbb564', padding: 40 }}>
            Загрузка...
          </div>
        ) : event ? (
          <div className="event-content">
            ...
            <div className="description-section">
              ...
              <p className="description-text">{event.description || 'Нет описания'}</p>
            </div>
            ...
            <div className="participants-section">
              ...
              <div className="participants-list-static">
                {participants.length === 0 && (
                  <span style={{ color: '#808080', fontSize: 14 }}>Нет участников</span>
                )}
                {displayedParticipants.map((p, index) => (
                  <div key={p.id || index} className="participant-pill">
          ...
          <div className="event-content" style={{ textAlign: 'center', color: '#808080', padding: 40 }}>
            Встреча не найдена
          </div>
        )}
      </div>

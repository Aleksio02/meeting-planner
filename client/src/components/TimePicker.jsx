...
const TimePicker = ({ options, onChange, className, style }) => {
  ...
  const handleSelect = (value) => { setInputValue(value); onChange(value); setIsOpen(false); };
  ...
  return (
    <div className="timepicker-container" ref={containerRef}>
      <input type="text" className={`timepicker-input ${className || ''}`} value={inputValue} onChange={handleInputChange} onFocus={handleFocus} placeholder="00:00" autoComplete="off" style={style} />
      {isOpen && filteredOptions.length > 0 && (
        <ul className="timepicker-list">
          {filteredOptions.map((time) => (
            <li key={time} className="timepicker-item" onClick={() => handleSelect(time)}>{time}</li>
          ))}
        </ul>
      )}
    </div>
